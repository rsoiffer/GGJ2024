...
public class EXPManager : MonoBehaviour
{
	...
	private void calcLevelUp(PokemonInstance mon){
		GrowthRate growthrate=mon.GetGrowthRate();
		int nextLevel=mon.level+1;
		float nextLevelMinExp=0;
		switch  (growthrate){
			//exp=6/5n^3-15n^2+100n-140
			//bulbapedia calls parabolic "medium slow"
			case GrowthRate.Parabolic:
			nextLevelMinExp=6.0f/5.0f*Mathf.Pow(nextLevel,3.0f)-15.0f*Mathf.Pow(nextLevel,2.0f)+100.0f*nextLevel-140.0f;
			...
			if (nextLevel<15){
				nextLevelMinExp=(Mathf.Pow(nextLevel,3.0f)*(Mathf.Floor((nextLevel+1.0f)/3.0f)+24))/50.0f;
			}
			...
			if (nextLevel<50){
				...
				nextLevelMinExp=(Mathf.Pow(nextLevel,3.0f)*Mathf.Floor((1911.0f-10.0f*nextLevel)/3.0f))/500.0f;
			}
			else{
